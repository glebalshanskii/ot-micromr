...
def _run_replications(
    ...
    list[dict[str, Any]],
    list[BookEventRecord],
    dict[tuple[float, bool, int], dict[str, float]],
    dict[str, tuple[int, ...]],
    dict[str, Any],
]:
    ...
    for result in results:
        ...
        if spec.experiment_id.startswith("SIM-MOMENTS"):
    ...
    return seed_rows, all_events, binned, stream_spawn_keys, execution
...
def _balanced_summary_v2(
    ...
    for epsilon in epsilons:
        ...
        old_flow_mean, old_flow_se, _, _ = _student_interval(
            [float(row["open_close_flow_signed_relative_residual"]) for row in rows]
        )
        summary: dict[str, Any] = {
            "epsilon": epsilon,
            "replication_count": len(rows),
            ...
            "legacy_conditional_flow_residual_mean": old_flow_mean,
            "legacy_conditional_flow_residual_se": old_flow_se,
    ...
    invariant_totals = {key: sum(int(row[key]) for row in seed_rows) for key in invariant_keys}
    generator_max = max(
    ...
    acceptance = {
        **{
            key: value <= int(acceptance_spec[f"{key}_max"])
            for key, value in invariant_totals.items()
        },
        ...
        "deterministic_replay_coverage": checked_replays == 3,
    }
    metrics = {
        "primary_resolution_epsilon": primary,
        ...
        "open_drift_diagnostics": {
            ...
            "role": "compatibility_only_exact_generator_is_the_gate",
        },
        "invariant_totals": invariant_totals,
...
def evaluate_simulation(spec: RunSpec, run_directory: Path) -> SimulationEvaluation:
    seed_rows, events, binned, stream_spawn_keys, execution = _run_replications(spec)
    settings = settings_from_spec(spec.values)
    if spec.experiment_id.startswith("SIM-MOMENTS-"):
        if spec.experiment_id.endswith("002"):
            resolution_rows, figure_rows, metrics, acceptance = _balanced_summary_v2(
                spec, seed_rows
            )
        else:
        ...
        if spec.experiment_id.endswith("002"):
            resolution_rows, figure_rows, metrics, acceptance = _unbalanced_summary_v2(
                spec, seed_rows
            )
        else:
            resolution_rows, figure_rows, metrics, acceptance = _unbalanced_summary(
                spec, seed_rows
            )
        binned_rows = None
