...
| `SIM-FIG4-002` | `sim_fig4_002.toml` | Powered independent Figure 4 reconstruction | completed; legacy interval gate superseded; corrected operational validity passed; scientific inconclusive |
...
`SIM-FIG4-002` retains its exact historical config, including the rejected all-cell interval
floor, so commit `ca9aa7c` can reproduce the original runner decision. ADR-0009 supersedes that
gate without changing the config or raw run. The canonical corrected decision is generated by:
